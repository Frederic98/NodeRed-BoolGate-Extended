    ...
    updateState(msg) {
        return new Promise((resolve, reject) => {
            for (let i = 0; i < this.data.length; i++) {
                ...
                if (rule.hasOwnProperty("validated")){
                    lastValidated = rule.validated;
                }
                //Check if the rule is ok
                if (msg) { // if there is a msg
                    if ((rule.propertyType === 'msg' && (msg.topic === rule.topic || rule.topic === '')) || rule.propertyType !== 'msg') { // if the rule match with the message got
                        if (operators[rule.t](test, v1, v2, rule.case, lastValidated)) {
                            rule.validated = true;
                        } else {
